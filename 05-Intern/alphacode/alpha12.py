...
for di in range(startdi,enddi+1):
    ...
    for ii in range(instruments):
        rank1[ii]=ops[di-delay,ii]-sum(vwap[di-10:di,ii])/10
        rank2[ii]=abs(close[di-delay,ii]-vwap[di-delay,ii])
    rank1 = pd.Series(rank1).rank()
    rank2 = pd.Series(rank2).rank()
    
    for ii in range(instruments):
        alpha[0][di-startdi][ii] = rank1[ii]*rank2[ii]*-1
